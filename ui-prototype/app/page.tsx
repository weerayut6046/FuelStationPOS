...
type DocumentDetail = DocumentSummary & {
  ...
  buyer_snapshot: { legalName?: string; taxId?: string; branchLabel?: string; address?: string; vehicleRegistration?: string; vehicleProvince?: string } | null;
  subtotal: string;
  vat_rate: string;
  vat_amount: string;
...
export default function Home() {
  ...
  const [saleVehicleRegistration, setSaleVehicleRegistration] = useState("");
  const [saleVehicleProvince, setSaleVehicleProvince] = useState("");
    ...
    try {
      ...
      if (saleDocumentType === "FULL_TAX_INVOICE" && customerEntryMode === "NEW") {
        const customerResponse = await fetch(`${apiUrl}/api/customers`, {
          ...
          body: JSON.stringify({ legalName: customerLegalName, taxId: customerTaxId, branchCode: customerBranchCode, address: customerAddress, vehicleRegistration: saleVehicleRegistration, vehicleProvince: saleVehicleProvince }),
        });
      ...
      const response = await fetch(`${apiUrl}/api/sales`, {
        ...
        body: JSON.stringify({
          ...
          customerId,
          vehicleRegistration: saleVehicleRegistration || undefined,
          vehicleProvince: saleVehicleProvince || undefined,
          items: [{
      ...
      <section className={`sale-layer ${saleOpen ? "open" : ""}`} aria-hidden={!saleOpen}>
        <form className="sale-dialog" onSubmit={(event) => void submitSale(event)}>
          ...
          {saleDocumentType === "FULL_TAX_INVOICE" && <div className="customer-panel">
            ...
            <div className="vehicle-fields"><label><span>ทะเบียนรถ</span><input value={saleVehicleRegistration} onChange={(event) => setSaleVehicleRegistration(event.target.value)} maxLength={30} placeholder="เช่น 3ขธ 1955" /></label><label><span>จังหวัด</span><input value={saleVehicleProvince} onChange={(event) => setSaleVehicleProvince(event.target.value)} maxLength={100} placeholder="เช่น กรุงเทพมหานคร" /></label></div>
          </div>}
  ...
  return (
    <article className="invoice-paper printable-document">
      ...
      <section className="invoice-info">
        <div><h4>ข้อมูลลูกค้า</h4><p><b>{document?.buyer_snapshot?.legalName ?? "ไม่ระบุผู้ซื้อ"}</b></p><p>เลขประจำตัวผู้เสียภาษี {document?.buyer_snapshot?.taxId ?? "—"} · {document?.buyer_snapshot?.branchLabel ?? "—"}</p><p>{document?.buyer_snapshot?.address ?? "—"}</p><p><b>ทะเบียนรถ</b> {document?.buyer_snapshot?.vehicleRegistration ?? "—"} · {document?.buyer_snapshot?.vehicleProvince ?? "—"}</p></div>
